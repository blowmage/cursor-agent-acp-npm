...
import type { TerminalClient } from '../../src/tools/terminal-manager';
...
// Mock TerminalClient
const createMockClient = (): TerminalClient => {
...
describe('Terminal Integration Tests', () => {
  ...
  describe('Resource management', () => {
    ...
    it('should use await using for automatic cleanup', async () => {
      ...
      {
        // Using block scope with await using
        await using terminal = await terminalManager.createTerminal(
          'session-1',
          {
            command: 'echo',
          }
        ...
        // Terminal is active within scope
        await terminal.currentOutput();
      }

      // Terminal should be automatically released after scope
      // Note: We can't test this directly without access to the handle's internal state,
      // but we can verify that release was called by trying to use it
